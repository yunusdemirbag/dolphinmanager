...
export async function POST(request: NextRequest) {
  try {
    ...
    const { access_token, api_key, expires_at } = apiKeysData;
    ...
    // Token süresi dolmuş veya 10 dakika içinde dolacaksa yenile
    if (tokenExpiresAt && (tokenExpiresAt <= now || (tokenExpiresAt.getTime() - now.getTime() < 10 * 60 * 1000))) {
      console.log('⚠️ Etsy token süresi dolmuş veya yakında dolacak, yenileniyor...');
      console.log('🕒 Token bitiş zamanı:', tokenExpiresAt.toISOString());
      console.log('🕒 Şu anki zaman:', now.toISOString());
      console.log('⏱️ Kalan süre (dakika):', tokenExpiresAt ? Math.round((tokenExpiresAt.getTime() - now.getTime()) / (60 * 1000)) : 'Bilinmiyor');
      
      try {
        // Token yenileme fonksiyonu
        async function refreshEtsyToken(shopId: string): Promise<string | null> {
          console.log(`♻️ Token yenileme başlatılıyor - shop ${shopId}...`);
        ...
        if (newToken) {
          console.log('✅ Token başarıyla yenilendi, yeni token kullanılacak');
          currentAccessToken = newToken;
        } else {
          console.error('❌ Token yenileme başarısız, mevcut token kullanılmaya devam edilecek');
        }
      } catch (refreshError) {
        console.error('❌ Token yenileme hatası:', refreshError);
        // Token yenileme başarısız olsa bile işleme devam et, belki token hala geçerlidir
      }
    }
    ...
    const CACHE_DURATION = 72 * 60 * 60 * 1000; // 72 saat cache (3 gün)
    
    // Memory cache için tip tanımlaması
    ...
    // Memory cache kontrolü (globalThis scope)
    const MEMORY_CACHE_KEY = `shipping_profile_memory_${shop_id}`;
    const memoryCache = (globalThis as any)[MEMORY_CACHE_KEY] as MemoryCacheItem | undefined;
    
    if (memoryCache && memoryCache.timestamp > Date.now() - CACHE_DURATION) {
      shippingProfileId = memoryCache.profile_id;
      console.log(`✅ Shipping profile memory cache'den alındı:`, {
        profile_id: shippingProfileId,
        cache_age_hours: ((Date.now() - memoryCache.timestamp) / (1000 * 60 * 60)).toFixed(1),
        shop_id: shop_id
      });
    } else {
    ...
    try {
      ...
      if (!shippingProfileId) {
        ...
        if (shippingProfileId) {
          ...
          // Paralel olarak cache'e kaydet
          adminDb.collection('shipping_cache').doc(SHIPPING_CACHE_KEY).set(cacheData)
            .then(() => console.log('💾 Shipping profile Firebase cache\'e kaydedildi'))
            .catch(err => console.error('❌ Firebase cache kaydetme hatası:', err));
          
          // Memory cache'e de kaydet
          (globalThis as any)[MEMORY_CACHE_KEY] = cacheData;
          console.log('💾 Shipping profile memory cache\'e kaydedildi');
        }
      }
      ...
      if (!shippingProfileId) {
        ...
        if (shippingResponse.ok) {
          ...
          if (shippingData.results && shippingData.results.length > 0) {
            ...
            // Paralel olarak işlemleri yap
            Promise.all([
              // Cache'e kaydet (hızlı erişim için)
              adminDb.collection('shipping_cache').doc(SHIPPING_CACHE_KEY).set(cacheData),
              
              // Firebase'e kalıcı kaydet
              adminDb.collection('shipping_profiles').add(profileData)
            ])
            .then(() => {
              console.log('💾 Shipping profile hem cache\'e hem Firebase\'e kaydedildi');
            })
            .catch(err => {
              console.error('❌ Shipping profile kaydetme hatası:', err);
            });
    ...
    // Processing Profiles - legacy=false parametresi ile yeni API kullanımı
    etsyFormData.append('legacy', 'false');
    
    // Processing Profile bilgilerini ekle - tüm fiziksel ürünler için readiness_state_id gerekli
    if (listingData.type !== 'download') {
      try {
        console.log('🔄 Processing Profile işlemi başlatılıyor...');
        console.log('📋 listingData.type:', listingData.type);
        
        // Readiness state belirleme - made_to_order veya ready_to_ship
        const readinessState = listingData.when_made === 'made_to_order' ? 'made_to_order' : 'ready_to_ship';
        console.log(`📋 Readiness state: ${readinessState}`);
        
        // Önce mevcut readiness state definition'ları al
        const getReadinessResponse = await fetch(`https://openapi.etsy.com/v3/application/shops/${shop_id}/readiness-state-definitions`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${currentAccessToken}`,
            'x-api-key': api_key
        ...
        if (getReadinessResponse.ok) {
          const readinessDefinitions = await getReadinessResponse.json();
          console.log(`📋 Mevcut readiness state definition sayısı: ${readinessDefinitions.count}`);
          
          if (readinessDefinitions.results && readinessDefinitions.results.length > 0) {
            // İlk readiness state definition'ı kullan
            const firstDefinition = readinessDefinitions.results[0];
            console.log(`✅ Mevcut Processing Profile kullanılıyor, readiness_state_id: ${firstDefinition.readiness_state_id}`);
            etsyFormData.append('readiness_state_id', firstDefinition.readiness_state_id.toString());
          } else {
            // Readiness state definition yoksa, readiness_state_id parametresini gönderme
            console.log('⚠️ Mağazada readiness state definition bulunamadı, readiness_state_id parametresi gönderilmeyecek');
            // readiness_state_id parametresini gönderme
          }
        } else {
          // API hatası durumunda readiness_state_id parametresini gönderme
          console.error('❌ Readiness state definition\'ları alırken hata oluştu, readiness_state_id parametresi gönderilmeyecek');
          // readiness_state_id parametresini gönderme
        }
      } catch (processingError: any) {
        console.error('❌ Processing Profile exception:', processingError);
        console.error('❌ Processing Profile hata mesajı:', processingError.message || 'Bilinmeyen hata');
        
        // Hata durumunda readiness_state_id parametresini gönderme
        console.warn('⚠️ Processing Profile hatası görmezden geliniyor, readiness_state_id parametresi gönderilmeyecek');
        // readiness_state_id parametresini gönderme
      }
    }
    ...
    if (listingData.type === 'download') {
      ...
      // Fiziksel ürünler için type değerini listingData'ya da ekleyelim
      listingData.type = 'physical';
      console.log('📝 listingData.type = physical olarak ayarlandı');
    }
    ...
    // ADIM 2.8: Varyasyonları arka planda ekle (yeni sistem)
    if (listingData.has_variations && listingData.variations?.length > 0) {
      console.log('📤 ADIM 2.8: Varyasyonlar arka planda ekleniyor...');
      console.log(`🎯 Toplam varyasyon sayısı:`, listingData.variations.length);
      
      // Varyasyonları arka planda ekle - ana işlemi bloklamadan
      (async () => {
        try {
          console.log(`🔄 Varyasyon ekleme işlemi arka planda başlatıldı - listing_id: ${etsyResult.listing_id}`);
          
          // Kısa bir bekleme ekle - listing'in tam oluşması için
          await new Promise(resolve => setTimeout(resolve, 1000));
          
          const startTime = Date.now();
          
          // readiness_state_id'yi al
          const readinessStateId = etsyResult.readiness_state_id;
          console.log(`🔄 Varyasyonlar için readiness_state_id kullanılıyor: ${readinessStateId}`);
          
          await addInventoryWithVariations(
            currentAccessToken,
            etsyResult.listing_id,
            listingData.variations,
            readinessStateId
          );
          
          const duration = ((Date.now() - startTime) / 1000).toFixed(1);
          console.log(`✅ Varyasyonlar başarıyla eklendi - 48 kombinasyon oluşturuldu (${duration}s)`);
          
          // Varyasyon ekleme başarılı olduğunda Firebase'e kaydet
          await adminDb.collection('etsy_listings').doc(etsyResult.listing_id.toString()).update({
            variations_added: true,
            variations_count: listingData.variations.length,
            variations_added_at: new Date()
          }).catch(err => console.error('❌ Varyasyon durumu kaydetme hatası:', err));
          
        } catch (variationError) {
          console.error('❌ Varyasyon ekleme hatası:', variationError);
          
          // Hata durumunda Firebase'e kaydet
          await adminDb.collection('etsy_listings').doc(etsyResult.listing_id.toString()).update({
            variations_error: true,
            variations_error_message: variationError instanceof Error ? variationError.message : 'Unknown error'
          }).catch(err => console.error('❌ Varyasyon hata durumu kaydetme hatası:', err));
        }
      })();
      
      console.log('⏱️ Varyasyon ekleme işlemi arka planda devam ediyor, ana işlem devam edecek');
    }
