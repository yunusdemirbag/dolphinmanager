...
export async function GET(request: NextRequest) {
  try {
    ...
    // Eğer hiç section yoksa, boş bir dizi döndür - mock data kullanmıyoruz
    if (sections.length === 0) {
      console.log('⚠️ Etsy\'de hiç kategori bulunamadı');
      
      // Kullanıcıya bilgi ver
      return NextResponse.json({
        sections: [],
        warning: 'Etsy mağazanızda hiç kategori bulunamadı. Lütfen önce bir kategori oluşturun.'
      });
    }
