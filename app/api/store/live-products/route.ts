    ...
    const productsWithImages = await Promise.all(
      listings.map(async (listing, index) => {
        try {
          ...
          // Sadece ilk resmi al - performans için
          console.log(`📸 Resim alınıyor ${index + 1}/${listings.length}: ${listing.listing_id}`);
          
          // Önce mevcut images dizisini kontrol et
          if (listing.images && listing.images.length > 0) {
            console.log(`✅ Ürün zaten resim içeriyor ${listing.listing_id}: ${listing.images.length} resim`);
            return {
              ...listing,
              images: [listing.images[0]], // Sadece ilk resmi tut
              has_images: true
            };
          }
          ...
          try {
            const imageResponse = await fetch(
              `${ETSY_API_URL}/application/listings/${listing.listing_id}/images`,
              {
                headers: {
                  'Authorization': `Bearer ${access_token}`,
                  'x-api-key': process.env.ETSY_API_KEY || process.env.ETSY_CLIENT_ID!,
                }
              }
            );
