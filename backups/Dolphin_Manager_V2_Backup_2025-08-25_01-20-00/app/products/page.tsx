...
async function getInitialProducts(userId: string) {
    try {
      ...
      // Her zaman önce Firebase'den ürünleri çek
      const productCount = await (await import('@/lib/firebase-sync')).countProductsInFirebaseAdmin(userId);
      console.log(`📊 Firebase'de ${productCount} ürün bulundu (Kullanıcı: ${userId})`);
      ...
      console.log(`🔄 Aktif mağaza bulundu: ${store.shop_name} (${store.shop_id}). Etsy API'den ürünler çekiliyor...`);
      ...
      try {
        ...
        if (products.length > 0) {
            ...
            try {
              ...
              const storeRef = adminDb.collection('etsy_stores').doc(String(store.shop_id));
              await storeRef.update({
                last_sync_at: new Date(),
                total_products: products.length
              });
              
              console.log(`📝 Mağaza senkronizasyon bilgileri güncellendi: ${store.shop_name}`);
            } catch (error) {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <ProductsPageClient 
        ...
        store={activeStore ? {
          shop_id: String(activeStore.shop_id),
          shop_name: activeStore.shop_name,
