...
export async function getProductsFromFirebaseAdmin(
  ...
  try {
    ...
    if (shopId) {
      ...
      if (isNaN(shopIdNumber)) {
        ...
        // Firestore'da compound query kısıtlaması nedeniyle,
        // shop_id veya shopId alanlarından sadece birini kullanabiliriz
        // Bu nedenle, shop_id alanını kullanıyoruz ve istemci tarafında ek filtreleme yapıyoruz
        try {
          // Önce shop_id ile deneyelim
          query = query.where('shop_id', '==', shopIdNumber);
          console.log(`🔍 'shop_id' alanı ile filtreleme yapılıyor: ${shopIdNumber}`);
        } catch (error) {
          console.error(`❌ 'shop_id' ile filtreleme hatası:`, error);
          
          try {
            // shop_id ile başarısız olursa, shopId ile deneyelim
            query = query.where('shopId', '==', shopIdNumber);
            console.log(`🔍 'shopId' alanı ile filtreleme yapılıyor: ${shopIdNumber}`);
          } catch (secondError) {
            console.error(`❌ 'shopId' ile filtreleme hatası:`, secondError);
            console.log(`⚠️ Filtreleme yapılamadı, tüm ürünler döndürülecek`);
          }
        }
      }
    }
  ...
  const products = (snapshot.docs || []).map(doc => {
    ...
    // Ürünün shop_id'sini kontrol et ve log'la
    if (shopId) {
      const shopIdNumber = parseInt(shopId);
      const productShopId = data.shop_id ? Number(data.shop_id) : null;
      const productShopId2 = data.shopId ? Number(data.shopId) : null;
      
      console.log(`🔍 Ürün kontrol: ID=${doc.id}, shop_id=${productShopId}, shopId=${productShopId2}, aranan=${shopIdNumber}`);
    }
