...
class AITitleTagSystem {
  ...
  private async createInitialCollections() {
    ...
    const titleTemplates: TitleTemplate[] = [
      ...
      },
      {
        id: 'landscape',
        category: 'landscape',
        template: '{location} {landmark} Canvas Wall Art Print, {style} {country} {timeOfDay} {scene}, Gift for {audience}',
        examples: [
          ...
          'New York Skyline Canvas Wall Art Print, USA Modern City Panorama, Gift for Urban Decor Enthusiasts'
        ],
        keywords: ['landscape', 'cityscape', 'landmark', 'travel', 'scenic', 'city', 'panorama', 'skyline', 'destination'],
        created_at: new Date(),
        success_count: 0
      },
      {
        id: 'floral',
        category: 'floral',
        template: '{color} {flower} {scene} Canvas Wall Art Print, {style} {season} {location} {detail}, Gift for {audience}',
        examples: [
          'Purple Lavender Field Canvas Wall Art Print, Vibrant French Countryside Landscape, Gift for Flower Lovers',
          'Pink Cherry Blossom Garden Canvas Wall Art Print, Serene Japanese Spring Botanical Scene, Gift for Nature Enthusiasts',
          'Colorful Tulip Garden Canvas Wall Art Print, Dutch Floral Landscape Morning View, Gift for Gardening Lovers',
          'Golden Sunflower Field Canvas Wall Art Print, Bright Summer Countryside Panorama, Gift for Botanical Decor',
          'Red Poppy Meadow Canvas Wall Art Print, Elegant Italian Countryside Sunset View, Gift for Floral Art Collectors'
        ],
        keywords: ['floral', 'flower', 'garden', 'botanical', 'bloom', 'petal', 'blossom', 'meadow', 'field', 'countryside'],
        created_at: new Date(),
        success_count: 0
      }
    ];
    ...
    const tagTemplates: TagTemplate[] = [
      ...
      },
      {
        id: 'landscape_tags',
        category: 'landscape',
        tags: ['travel wall art', 'landmark print', 'cityscape art', 'city skyline', 'destination art', 'travel gift', 'scenic view', 'panoramic art', 'famous landmark', 'city view', 'travel decor', 'wanderlust art', 'location print'],
        title_patterns: ['paris', 'london', 'new york', 'istanbul', 'santorini', 'venice', 'rome', 'tokyo', 'barcelona', 'amsterdam', 'prague', 'sydney', 'dubai', 'eiffel', 'tower', 'bridge', 'skyline', 'cityscape', 'landmark'],
        created_at: new Date(),
        success_count: 0
      },
      {
        id: 'floral_tags',
        category: 'floral',
        tags: ['flower wall art', 'floral canvas', 'botanical print', 'garden art', 'flower field', 'lavender art', 'tulip print', 'sunflower decor', 'cherry blossom', 'rose garden', 'flower gift', 'botanical decor', 'floral home art'],
        title_patterns: ['lavender', 'tulip', 'sunflower', 'rose', 'poppy', 'cherry blossom', 'peony', 'daisy', 'wildflower', 'garden', 'floral', 'flower', 'botanical', 'bloom', 'meadow', 'field'],
        created_at: new Date(),
        success_count: 0
      }
    ];
    ...
    try {
      ...
      if (template) {
        ...
        if (category === 'rothko') {
          ...
          // Landscape/Travel specific variables
          const locations = ['Paris', 'London', 'New York', 'Istanbul', 'Santorini', 'Venice', 'Rome', 'Tokyo', 'Barcelona', 'Amsterdam', 'Prague', 'Sydney', 'Dubai'];
          const landmarks = ['Eiffel Tower', 'Big Ben', 'Skyline', 'Bosphorus', 'Blue Domes', 'Grand Canal', 'Colosseum', 'Mount Fuji', 'Sagrada Familia', 'Canals', 'Castle', 'Opera House', 'Burj Khalifa'];
          const styles = ['Romantic', 'Modern', 'Historic', 'Vibrant', 'Serene', 'Colorful', 'Elegant', 'Dramatic', 'Peaceful', 'Majestic'];
          const countries = ['France', 'UK', 'USA', 'Turkey', 'Greece', 'Italy', 'Japan', 'Spain', 'Netherlands', 'Czech Republic', 'Australia', 'UAE'];
          const timeOfDay = ['Sunset', 'Night', 'Morning', 'Dusk', 'Dawn', 'Twilight', 'Daytime', 'Evening'];
          const scenes = ['Cityscape', 'Skyline', 'Panorama', 'View', 'Scene', 'Landscape', 'Seascape', 'Urban Scene'];
          const audiences = ['Travel Lovers', 'City Enthusiasts', 'Urban Decor', 'Wanderlust Decor', 'Mediterranean Lovers', 'European Style', 'Asian Decor', 'Adventure Seekers', 'World Travelers', 'Destination Collectors'];
          ...
          randomVariables = `location="${randomLocation}", landmark="${randomLandmark}", style="${randomStyle}", country="${randomCountry}", timeOfDay="${randomTimeOfDay}", scene="${randomScene}", audience="${randomAudience}"`;
        } else if (category === 'floral') {
          // Floral/Garden specific variables
          const colors = ['Purple', 'Pink', 'Red', 'Yellow', 'White', 'Blue', 'Orange', 'Colorful', 'Vibrant', 'Pastel'];
          const flowers = ['Lavender', 'Cherry Blossom', 'Tulip', 'Sunflower', 'Rose', 'Poppy', 'Peony', 'Daisy', 'Wildflower', 'Orchid', 'Lily', 'Hydrangea'];
          const scenes = ['Field', 'Garden', 'Meadow', 'Landscape', 'Botanical Scene', 'Countryside', 'Arrangement', 'Bouquet', 'Close-up', 'Panorama'];
          const styles = ['Vibrant', 'Serene', 'Elegant', 'Bright', 'Dreamy', 'Rustic', 'Delicate', 'Bold', 'Romantic', 'Peaceful'];
          const seasons = ['Spring', 'Summer', 'Autumn', 'Seasonal', 'Blooming', 'Morning', 'Sunset'];
          const locations = ['French Countryside', 'Japanese Garden', 'Dutch Landscape', 'Italian Countryside', 'English Garden', 'Provence', 'Tuscany', 'Kyoto', 'Amsterdam', 'Botanical Garden'];
          const details = ['Landscape', 'Botanical View', 'Floral Scene', 'Garden View', 'Nature Art', 'Blooming Flowers', 'Countryside View'];
          const audiences = ['Flower Lovers', 'Nature Enthusiasts', 'Gardening Lovers', 'Botanical Decor', 'Floral Art Collectors', 'Spring Decor', 'Garden Enthusiasts', 'Cottage Style', 'Country Home'];
          ...
          randomVariables = `color="${randomColor}", flower="${randomFlower}", scene="${randomScene}", style="${randomStyle}", season="${randomSeason}", location="${randomLocation}", detail="${randomDetail}", audience="${randomAudience}"`;
        }
  ...
  private async detectImageCategory(imageBase64: string): Promise<string> {
    try {
      // Önce manzara/çiçek analizi API'sini çağır
      try {
        console.log('🏞️ Manzara/çiçek analizi API çağrısı yapılıyor...');
        const formData = new FormData();
        formData.append('image', new Blob([Buffer.from(imageBase64, 'base64')], { type: 'image/jpeg' }));
        
        const landscapeResponse = await fetch('/api/ai/analyze-landscape', {
          method: 'POST',
          body: formData
        });
        ...
        if (landscapeResponse.ok) {
          ...
          console.log('🏞️ Manzara/çiçek analizi sonucu:', landscapeData);
          
          if (landscapeData.isLandscape) {
            console.log('✅ Görsel manzara/çiçek olarak tespit edildi!');
            
            // Çiçek/bahçe sahnesi mi kontrol et
            if (landscapeData.sceneType &&
                (landscapeData.sceneType.toLowerCase().includes('floral') ||
                 landscapeData.sceneType.toLowerCase().includes('flower') ||
                 landscapeData.sceneType.toLowerCase().includes('garden') ||
                 landscapeData.sceneType.toLowerCase().includes('botanical'))) {
              console.log('🌸 Görsel çiçek/bahçe olarak tespit edildi!');
              return 'floral';
            }
          ...
          console.log('⚠️ Görsel manzara/çiçek olarak tespit edilmedi, normal kategori tespitine devam ediliyor...');
        }
      } catch (error) {
        console.error('❌ Manzara/çiçek analizi hatası:', error);
        // Hata durumunda normal kategori tespitine devam et
      }
      
      // Normal kategori tespiti
      const response = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          {
