...
const productCache: {
  [key: string]: {
    data: { products: any[], nextCursor: number | null },
    timestamp: number
  }
} = {};
...
export async function getProductsFromFirebaseAdmin(
  ...
  try {
    ...
    const cacheKey = `products_${userId}_${pageSize}_${startAfterListingId}_${shopId || 'all'}`;
    
    // Önbellekten veri kontrolü - önbellek süresini 5 dakikaya çıkardık
    const cachedData = productCache[cacheKey];
    if (cachedData && Date.now() - cachedData.timestamp < 300000) { // 5 dakika önbellek süresi
      console.log(`🚀 Önbellekten ${cachedData.data.products.length} ürün yüklendi`);
    ...
    if (shopId) {
      ...
      if (isNaN(shopIdNumber)) {
        console.error(`⚠️ Geçersiz shopId formatı: ${shopId}, filtreleme yapılmayacak`);
      } else {
        // Client-side filtreleme kullan - Firebase indeks hatası nedeniyle
        console.log(`🔍 Client-side filtreleme kullanılacak: ${shopIdNumber}`);
      }
    }
    
    // Sayfa boyutunu sınırla - çok büyük veri kümeleri için
    const effectivePageSize = Math.min(pageSize, 200); // Maksimum 200 ürün
    query = query.orderBy('listing_id', 'desc').limit(effectivePageSize);
    ...
    // Sorgu zaman aşımını önlemek için timeout ekle
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Sorgu zaman aşımı')), 15000)
    );
    
    // Promise.race ile sorgu veya timeout'tan hangisi önce tamamlanırsa onu al
    const snapshotPromise = query.get();
    const snapshotResult = await Promise.race([snapshotPromise, timeoutPromise])
      .then(result => result)
      .catch(error => {
        console.error('⏱️ Sorgu zaman aşımı veya hata:', error);
        // Zaman aşımı durumunda boş sonuç döndür
        return { docs: [] };
      });
    ...
    console.log(`📦 ${snapshot.docs?.length || 0} ürün bulundu`);

    // Bellek optimizasyonu için veri işleme
    const products = [];
    
    // Batch işleme - bellek kullanımını optimize etmek için
    for (let i = 0; i < (snapshot.docs?.length || 0); i++) {
      const doc = snapshot.docs[i];
      const data = doc.data();
      
      // Client-side filtreleme - eğer shopId belirtilmişse
      if (shopId) {
        const shopIdNumber = parseInt(shopId);
        const productShopId = data.shop_id ? Number(data.shop_id) : null;
        const productShopId2 = data.shopId ? Number(data.shopId) : null;
        
        // ShopId eşleşmiyorsa bu ürünü atla
        if (productShopId !== shopIdNumber && productShopId2 !== shopIdNumber) {
          continue;
        }
      }

      // Sadece gerekli alanları al - bellek kullanımını azaltmak için
      ...
      products.push(serializableData);
    }
    ...
    if (cacheKeys.length > 20) { // Maksimum 20 önbellek girişi
      // En eski önbelleği bul ve sil
      let oldestKey = cacheKeys[0];
      let oldestTime = productCache[oldestKey].timestamp;
      
      for (const key of cacheKeys) {
        if (productCache[key].timestamp < oldestTime) {
          oldestTime = productCache[key].timestamp;
          oldestKey = key;
        }
      }
      
      delete productCache[oldestKey];
      console.log(`🧹 Önbellek temizlendi: ${oldestKey}`);
    }

    console.log(`✅ Ürünler hazırlandı - nextCursor: ${nextCursor}, ${products.length} ürün`);
    return result;
  } catch (error) {
    ...
    // Hata durumunda boş sonuç döndür - uygulamanın çökmesini önle
    return { products: [], nextCursor: null };
  }
}
