...
class StoreDataSystem {
      ...
      for (let i = 0; i < allListings.length; i += batchSize) {
        ...
        for (const listing of batch) {
          try {
            ...
            // Başlık ve etiketlerin ortak kütüphaneye eklenmesi devre dışı bırakıldı
  ...
  /**
   * Update ORTAK AI title library with new title - Tüm mağazalar kullanır!
   */
  private async updateTitleLibrary(title: string): Promise<void> {
    try {
      // ORTAK kütüphane - tüm mağazalar için tek kayıt
      await adminDb!.collection('shared_ai_library').add({
        type: 'title',
        content: title,
        created_at: new Date(),
        source: 'multi_store_sync',
        usage_count: 1,
        last_used: new Date()
      });
      
      console.log('📚 Ortak başlık kütüphanesine eklendi:', title.substring(0, 50) + '...');
    } catch (error) {
      console.error('Error updating shared title library:', error);
    }
  }
  
  /**
   * Update ORTAK AI tag library with new tags - Tüm mağazalar kullanır!
   */
  private async updateTagLibrary(tags: string[]): Promise<void> {
    try {
      if (tags && tags.length > 0) {
        // Her tag için ayrı kayıt - daha iyi analiz için
        ...
        console.log('🏷️ Ortak tag kütüphanesine eklendi:', tags.length, 'tag');
      }
    } catch (error) {
      console.error('Error updating shared tag library:', error);
    }
  }
